...
Profile 0.17 source
  ...
  -> Forge diagnosis and bounded exact repair
...
correlates identities, applies authorized exact edits, and preserves layered
truth. A test `FAIL` with unavailable debug evidence remains `FAIL` plus debug
`UNKNOWN`.
