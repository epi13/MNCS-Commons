...
`matches_discriminants` is its exact Python mirror. Mixed-evidence records
project to the strongest asserted outcome (PASS > FAIL > UNKNOWN) on both
...
portable-wasm: 243 cases x 2 backends, all PASS. The Python mirrors agree
with every checked-in expectation (`test_mesh_interop.py`), and the
mncs-actions `commons-mesh` check re-verifies versions, corpora drift,
golden digest, kernel studies, and descriptor negotiation on every run.
